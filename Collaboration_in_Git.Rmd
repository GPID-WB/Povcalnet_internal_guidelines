...
One of the most important habits when working with projects in Git is to commit *often* and *early*. The reason for this is at a commit should only contain changes related to a single problem. If you should discover a new problem, you should do a separate commit. This is much easier to achieve if start out by committing changes early on in the project. It is however of course also important that the commit solves the problem that it claims to solve, so doing an extra code check before you commit is a good idea. 
...
A good commit message should be concise, but also add the details necessary for people to understand what the commit does. This is particularly important when working in a shared repository (but it could also be important for your future self!). Somebody should be able to look at your commit message and get an instant sense of *why* the commit was implemented. It is important to start your commit message with a short one line summary of maximum 50 characters. This should preferably be written in the imperative. An example how to structure a good commit message is given below (taken from this [blog post](https://chris.beams.io/posts/git-commit/#seven-rules))[^writing-a-good-commit-message]. 
...
You can amend a previous commit if needed, for example if you made a significant typo and want to correct it or if you want to add a more detailed explanation to your commit message. You can also use amend to add more changes to a previous commit, but depending on how large those changes are you could also consider just add a new commit. It is also important to not amend commits that you have already pushed to a shared repository. 
...
If have some uncommitted changes that you don't want to commit, but still want to save them for later use you can use the Stash option. The two commands for this is SmartGit are "Save Stash" and "Apply Stash", shown in the icons below. You can think of this as putting your changes in a box (with Save Stash) and then opening this box at later if you need to (with Apply Stash). 
...
This easiest "undo" operation in Git is to unstage a file you have already prepared for a commit. This is useful if you realize that the file needs some more work before solving the problem or adding the feature you are working on. Remember that it is always a good idea to check that your code is working before committing to your local branch. If you find an error at this step of the workflow just unstage the file. In SmartGit this a file can be done by clicking the below icon. Note however that unstaging will not help you if the commit has already been made. 
...
Reset is one of the options available if you want to undo changes in Git. It can be used to either reset files in your staging index (this is same as unstage) or resetting commits in your local repository. 
...
**It is very important to never use Git reset on a commit that has been published to a shared repository**. This can create serious problems when your collaborators try to sync up with your repository. If you find yourself in a situation where you want to "undo" a commit in a shared repository you should always use git *revert*, not reset. In general git revert can be seen as a safer option (see below). 
...
Revert is another option for undoing changes in Git, although it is not a traditional undo operation. A git revert *inverts* the changes introduced by a commit and *appends a new commit* with the resulting inverse content. This is what makes the revert operation safer than a git reset. By adding a new commit git revert ensures that all the project's history is kept in place.[^git-revert]

An other important difference between revert and reset is that git revert is able to target an individual commit at an arbitrary point in the history, whereas git reset can only work backward from the current commit.

You can use following icon in SmartGit to do a git revert. 
...
- History of files
- why you shouldn't modify the git history 
<!--- Other things -->

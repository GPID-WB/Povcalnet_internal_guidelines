...
In case you have staged a file (preparing it for a commit), but realize that the file need some more work it is easy to unstage the file. In SmartGit this can be done by clicking the below icon. 
...
Reset is one of the options available if you want to undo changes in Git. It can be used to either reset files in your staging index (this is same as unstage) or reset commits in your local repository. 

It can however be a complex tool to use. There are important differences between doing a soft, mixed or hard reset. Particularly a hard reset can be dangerous since it involves resetting your staging environment and working directory to specific commit (a point in time on your Git history tree). This implies that any pending work in your index and working directory will be lost. It therefore important to be careful when use git reset on your commit history, and to [read up on the different options](https://www.atlassian.com/git/tutorials/undoing-changes/git-reset) before doing so. 

*It is also very important to never use Git reset on a commit that has been published to a shared repository*. This can create serious problems when your collaborators try to sync up with your repository. If you find yourself in a situation were you want to "undo" a commit in a shared repository you should always use git *revert*, not reset. In general git revert can also be seen as a safer option (see below). 


**- Revert**

Inverts the changes introduced by a commit and appends a new commit with the resulting inverse content. Note that since this keeps the history 
of the previous commit this is something you can do in a shared repository.  
